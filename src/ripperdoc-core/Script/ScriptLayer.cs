...
public sealed class ScriptLayer
{
    ...
    public IReadOnlyList<MethodContest> Methods =>
        _byMethod.Values.OrderBy(contest => contest.Method.Display, StringComparer.Ordinal).ToList();

    /// <summary>Methods that more than one source replaces.</summary>
    public IReadOnlyList<MethodContest> Contested =>
        Methods.Where(contest => contest.IsContested).ToList();

    /// <summary>Methods that at least one source wraps.</summary>
    ...
    /// <summary>
    /// Wraps whose body never calls the method they wrap.
    /// </summary>
    /// <remarks>
    /// Measured to compile with no error and no warning. At run time such a
    /// wrap ends the chain, so whatever it encloses does nothing.
    /// </remarks>
    ...
    /// winner this state reports then says it could be displaced.
    /// </param>
    ...
    public static ScriptLayer Of(ScriptEnumeration enumeration, IReadOnlyList<ScriptFileReading> readings)
    {
        ...
        foreach (var reading in readings.OrderBy(reading => reading.Source.Rank))
        {
            ...
            {
                var into = annotation.Kind == ScriptAnnotationKind.ReplaceMethod ? replacements : wraps;
        ...
        foreach (var method in replacements.Keys.Concat(wraps.Keys).Distinct())
        {
