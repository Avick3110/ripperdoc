...
| 21 | What a **running** deployment manager changes on disk that a stopped one does not | Wave 5 part two. The ruling's contract says a database in use is detected and refused or caveated by name, never read through silently. A LevelDB directory always carries a `LOCK` file and its presence proves nothing; measuring what a running manager changes needs the manager running, and the build session starts nothing | **OPEN** - the reader ships with the in-use arm **labelled unmeasured on every reading**, so a caller is told it was neither detected nor ruled out rather than left to assume it was checked. No check is written for it, because a check written now would assert a guess. The hypothesis to refute is published in [the addendum](findings/2026-09-02-manager-state-format-and-selection.md) §2: that the platform holds an exclusive lock on `LOCK` while the manager runs. Closed by one measurement window with the manager up, running the read-only probes that addendum lists - and the check for the arm is written when it lands |
