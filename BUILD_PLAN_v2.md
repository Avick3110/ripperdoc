...
most changes how the tool is actually used. Two hard requirements the research
phase produced:
...
- **A cycle check over ordering metadata belongs here, before any judgement
  does.** A collection whose ordering rules contain a cycle was observed to
  interrupt deployment with a large share of its mods never enabled. The cycle
  is detectable before a single file moves.
