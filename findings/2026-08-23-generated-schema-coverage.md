# A schema generated from the game explains three more shipped values than the inherited one, types 1,234 references it cannot, and is wrong about 100 slots it is right about
...
> shipped values contradict the generated schema on **100 field slots** and the
...
stored in. **66 of the 100** are that family: the accessor gives a plain name
where the stored value is a localisation key. The remaining **34** are
scattered across implausible pairings — a spatial vector against a name, a
boolean against an integer — with no family among them, which is the signature
of an identifier arithmetic collision rather than of a schema error.
...
| Individual references followed | **2,257,643** |
| Naming a record of the permitted kind, or one deriving from it | **1,885,122** |
| Naming a record of some other kind | **57** |
| Naming no record in this database at all | **372,464** |

**57 of 1,885,179 resolvable references — 0.003 % — name a kind the field does
...
of record" therefore has a floor of 57 on vanilla data, and that is a property
of the data rather than of the check.

The 372,464 that name no record are counted apart from those, because a
...
inspection rather than by a run is named where it appears: the 57 mismatched
references were characterised from a sample of their inheritance chains, not
all 57.
...
- **The 57 mismatched references being a derivation error** would have shown
  the stored kind deriving from the permitted one after all. Checked on a
  sample: they meet only at the root that every record type shares.
...
- **The dump describes the install it was taken from.** It was captured from a
  ...
  runtime.
- **The 100 contradicted slots are counted, not individually adjudicated.** The
  66 in the localisation-key family were identified as a family; the other 34
  were characterised by the shape of their type pairings rather than examined
  one by one.
